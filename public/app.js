...
function getResults() {
  ...
  $.getJSON("/all", function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      $("#results").prepend(`<a href="#" class='dataentry list-group-item' data-id=${data[i]._id}><span class='dataTitle' title="click to edit" data-id=${data[i]._id}>${data[i].title} [${data[i].reminder}]</span> <i class="glyphicon glyphicon-trash" title="click to delete"></i></a>`);
    }
  });
}
...
$(document).on("click", "#makenew", function() {
  ...
  .done(function(data) {
    ...
    $("#results").prepend(`<a href="#" class='dataentry list-group-item' data-id=${data._id}><span class='dataTitle' data-id=${data._id} title="click to edit">${data.title} [${data.reminder}]</span> <i class="glyphicon glyphicon-trash" title="click to delete"></i></a>`);
...
$(document).on("click", ".dataTitle", function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#actionbutton").html("<button id='updater' class='btn btn-info' data-id='" + data._id + "'>Update</button> <button id='cancel' class='btn btn-warning'>Cancel</button>");
    }
  });
});
...
$(document).on("click", "#updater", function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      $("#actionbutton").html("<button id='makenew' class='btn btn-success'>Submit</button> <button id='cancel' class='btn btn-warning'>Cancel</button>");
